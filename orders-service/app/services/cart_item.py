...
def create_cart_item(
    data: CartItemCreate, user_id: int, db: Session
    ...
    new_cart_item = CartItemResponse(
        id=cart_item.id, product=product, quantity=data.quantity
    )

    return new_cart_item


def list_cart_items(user_id: int, db: Session) -> list[CartItemResponse]:
    ...
    for item in cart_items:
        ...
        cart_item_response = CartItemResponse(
            id=item.id, product=product, quantity=item.quantity
        )
...
def change_cart_item_quantity(
    cart_item_id: int, data: CartItemUpdateQuantity, user_id: int, db: Session
) -> CartItemResponse | None:
    ...
    updated_cart_item = CartItemResponse(
        id=cart_item.id, product=product, quantity=cart_item.quantity
    )
...
def get_cart_item_by_id(
    cart_item_id: int, user_id: int, db: Session
) -> CartItemResponse | None:
    cart_item = cart_item_repo.get_cart_item_by_id(cart_item_id, db)
    if not cart_item:
        return None
