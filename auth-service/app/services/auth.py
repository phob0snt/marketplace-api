...
def authenticate_user(login_data: AccountLogin, db: Session):
    ...
    return AccountResponse(
        id=user.id, login=user.login, token_pair=create_token_pair(user)
    )
...
def create_token_pair(user: AuthORM) -> TokenPair:
    ...
    token_data = UserData(login=user.login, user_id=user.id, role=user.role)
...
def register_user(register_data: AccountCreate, db: Session):
    user = account_repo.get_account_by_login(register_data.login, db)

    if user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Login is already used"
        )
...
def register_user_with_login(
    register_data: AccountCreate, db: Session
) -> AccountResponse:
